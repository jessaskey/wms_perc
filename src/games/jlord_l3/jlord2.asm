...
;* $50 - Multiplier Loop
;* $60 - Lock Lit
;* $70 - Right Drop Target Timer
;* $80 -
;* $90 - Lower Loop Switch
;* $A0 - Min Tilt
;* $B0 - Fancy MB
;* $C0 - Ballshooter
;* $D0 - 
...
;******************************************************
;* Lamp Animation Data:  If minus goto next lamp 
;*                       immediately, if not, wait 2 IRQ's)
;******************************************************
ani_bt_data
	.db	$19,$95,$16,$BC,$BD,$BE,$3F,$93,$14,$B0,$87,$26,$B1,$88,$27,$B2
	.db	$89,$28,$B3,$8A,$29,$B4,$8B,$2A,$35,$36,$37,$B8,$BA,$BB,$1C,$B9
	.db	$12,$18,$91,$1D,$2B,$1E,$1E,$1E,$90,$2D,$A0,$A1,$A2,$A3,$24,$8F
	.db	$8E,$8D,$0C,$00
	
ani_tb_data
	.db	$8C,$8D,$8E,$0F,$A4,$A3,$A2,$A1,$20,$AD,$10,$10,$10,$1E,$2B,$9D
	.db	$11,$18,$92,$39,$9C,$BB,$BA,$38,$37,$36,$35,$AA,$8B,$34,$A9,$8A
	.db	$33,$A8,$89,$32,$A7,$88,$31,$A6,$87,$30,$94,$13,$BF,$BE,$BD,$3C
	.db	$96,$15,$19,$00
...
lamp_str_on	
		begin
			begin
				...
				anda #$7F
				stx  temp3
				jsr  lamp_on_1	;Turn on lamp?
				tstb 
			plend
			jsr  addthread	;Loop it with delay of 2 IRQ's
			.db  $01    
		loopend
lamp_str_rts
		rts
...
;********************************************************			
sw_leftdrain
sw_rightdrain	POINTS_(5,1000)			;5000 Points/Digit Sound
			EXE_
				ldab	#$02
			EXEEND_
      		JSRDR_(add_bonus_m)			
			IFEQR_($E0)			      ;BNER_RAM$00
      		...
      		ELSE_
      			JSRR_(gi_off_inc)
      			;no drainshield, see if they were magna-saving...
      			IFEQR_($FB,$F8,$30,$F8,$31)	;BNER_(SW#49 || SW#50) 
      				;yes, they were, make them feel bad
      				EXE_
      					jsr	get_random
						anda	#$07
						ifeq
							;Do the BlackKnight Laugh
							ldaa	#$05
						else
							;Regular JL speech
							ldaa	#$04
      			...
      			ELSE_
      				SSND_($18)
      				;no, just show drain animation
      				RCLR1_($08)				;Effect: Range #08
					RSET1_($48)				;Effect: Range #08
					JSRD_(ani_topbot)	
					RCLR1_($48)				;Effect: Range #08
					RCLR1_($08)				;Effect: Range #08
      			ENDIF_
      			JSRR_(gi_on_dec)
			ENDIF_
...
sw_ballshooter	IFEQR_($F8,$E0)    	      ;BNER_SW#E0
                  ...
                  ENDIF_
                  EXE_
			      NEWTHREAD(bottop)		;Spawn the new thread for 12345 logic
			EXEEND_
...
mb_fancy		BITON_($40)				;Turn ON: Bit#00
			...
			REMTHREADS_($FF,$43)		;Kill the lamp effects
;************************************************************
...
sw_1_target
sw_2_target
sw_3_target		POINTS_(1,1000)			;1000 Points
			...
			IFEQR_($F6,$04)		      ;BNER_RangeON#04
      			...
      			SSND_($1D)				;Sound #1D
      			RCLR0_($04)				;Effect: Range #04
      			RCLR1_($08)				;Effect: Range #08
      			JSRD_(alllamps_off)			
      			SETRAM_(rega,$1E)			;RAM$00=$1E
      			BEGIN_
	 				RINV1_($04)				;Effect: Range #04
	      			SLEEP_(3)
	      			ADDRAM_(rega,-1)			;RAM$00-=1
      		...
      		ELSE_
      			EXE_
					...
					pula
				EXEEND_
      			;flash the proper lamp a bunch, fancy like
      			SETRAM_(regb,$10)			;RAM$01=$10
      			BEGIN_
	 				BITINVP_(rega)			
	      			SLEEP_(3)
	      			ADDRAM_(regb,-1)			;RAM$01-=1
...
;*                     shield on loop.
;************************************************************
sw_5_rollover	JSRDR_(spawn_loop)
			EXE_
			      NEWTHREAD(sw_12345_com)		;Spawn the new thread for 12345 logic
			EXEEND_
			JSRDR_(add_bonus)		      ;1 bonus advance	
			KILL_
			
			
;***************************************************************
;* #4 Rollover switch: Will turn on multiplier lamp or drain
;*                     shield on loop and flash eject hole for
;*			     double kick.
;***************************************************************						
sw_4_rollover	JSRDR_(spawn_loop)		;Turn on the appropriate loop lamp	
			EXE_
			      NEWTHREAD(sw_12345_com)		;Spawn the new thread for 12345 logic
			EXEEND_	
			JSRDR_(add_bonus)			;1 bonus advance	
...
activate_loop	jsr	macro_start
			...
			IFEQR_($F6,$01)			;BNER_RangeON#01
				;here if all multipliers are lit
...
sw_left_magnet	IFNER_($FB,$FB,$F5,$06,$6D,$4F)	;BEQR_(BIT#0F || (BIT#2D || RangeOFF#06))
				PRI_($09)				;Priority=#09
				;This code removed to fix the left mag bug
...
;* Right Magnet Button
;**********************************************************************
sw_right_magnet	BEQR_($FB,$FB,$F5,$05,$6E,$4F,kill_2)	;BEQR_(BIT#0F || (BIT#2E || RangeOFF#05)) to kill_2
			PRI_($09)				;Priority=#09
...
hook_playerinit	ldaa	comma_flags
			...
			IFNER_($61)			      ;BEQR_BIT#21
      		...
      		ENDIF_
			RCLR1_($47)				;Effect: Range #07
			JSR_(add_drainshield)			
			IFEQR_($FC,$FF,$0F,$D7,$01)	;BNER_(ADJ#7 & LAMP#0F(D))==#1
				RCLR0_($06,$05)			;Effect: Range #06 Range #05
			ENDIF_
			...
			BITOFF_($7F)			;Clear the full-bonus flag
			BITOFF_($7E)			;Clear the bonus effect flag
			BITOFF_($7D)			;Clear the lamp animation flag
...
bg_snd		jsr	macro_start
			...
			BEGIN_
				...
				BEGIN_
					SLEEP_(255)
					SLEEP_(192)
					;do the magnasave sparkle effect
					RCLR1_($45,$05,$46,$06)			
					RSET1R1_($45,$46)			
					BEGIN_
						SLEEP_(4)
						RROR1_($45,$46)			
						EXE_				;CPU Execute Next 4 Bytes
							ldaa	$20
							anda	#$80
						EXEEND_
					NEEND_($FC,$E0,$00)
...
bonuseffect		jsr	macro_start
			IFNER_($7F)				;Effect Flag
				BEGIN_
					SLEEP_(4)
				NEEND_($FB,$7E,$7D )
				BITON_($7E)
				BEGIN_
					SLEEP_(2)
					IFNER_($FB,$FA,$3B,$F6,$00,$4F) ;BEQR_(BIT#0F || (RangeON#00 && BIT#FFFFFFFB))
						IFNER_($F6,$00)	
							;IFNER_($7D)				
								RCLR1_($40,$00)			;Effect: Range #C0 Range #00
								RSET1R1_($40)				;Effect: Range #40
								BEGIN_
									SLEEP_(3)
									RROR1_($40)			;Effect: Range #40
									EXE_				;CPU Execute Next 4 Bytes
										ldaa	$27
										anda	#$01
									EXEEND_
						...
						ELSE_
							EXE_
				      		...
				      		;IFNER_($7D)
								BEGIN_
									SLEEP_(3)
									RCLR1L0_($00)				;Effect: Range #00
								EQEND_($F5,$00)		;BNE_RangeOFF#00
							;ENDIF_
							;RCLR0_($00)
			      	      ENDIF_
				     	ELSE_
				     		;we are here if the bonus is at full count...
				     		IFNER_($7F)
					     		BITON_($7F)
					    		EXE_
			      			      NEWTHREAD(fullbonus)		;Do a fancy animation
			      			EXEEND_
			      		ENDIF_
			      	ENDIF_
			      	ADDRAM_(regb,-1)
			      EQEND_($FC,$E1,$00)		;BNER_RAM$01==$00
		  	ENDIF_
	      	BITOFF_($7E)
	      	KILL_
...
fullbonus		psha	
			...
			NEEND_($7E)
			PRI_($F0)				;Priority=#F0
			RCLR0_($00,$03)				;Clear Buffer 1
			RCLR1_($40,$43,$00,$03)				;Clear Buffer 1
			BEGIN_
				SETRAM_(rega,$2F)			;RAM$00=$2F
				BEGIN_
					SLEEP_(4)
					ADDRAM_(rega,$01)			;RAM$00+=$01
					BITINVP_(rega)			;Toggle Lamp/Bit @RAM:00
				EQEND_($FC,$E0,$38)		;BNER_RAM$00==$78
				BITINV_($3A)			;Toggle: Lamp#3A(bonus_20)
				BITINV_($3B)			;Toggle: Lamp#3B(bonus_30)
				SLEEP_(4)
				BITINV_($39)			;Toggle: Lamp#39(bonus_10)
				SLEEP_(4)
			LOOP_
...
hook_outhole	jsr	cpdisp_show
			...
			jsr	macro_start
			BEGIN_
				SLEEP_(10)				;Wait for bonus effects to end
			NEEND_($7E)
			REMTHREADS_($F8,$D0)		;Remove Multiple Threads Based on Priority
			REMTHREADS_($F8,$F0)		;Remove Multiple Threads Based on Priority
			JSRD_(send_sound)		
			;see if they had full bonus...
			IFEQR_($7F)
				;turn on 9 and 30
				RCLR0_($43,$40)
				BITON_($38,$3B)
			...
			IFNER_($FC,$E1,$00)	      ;BEQR_RAM$01==#0
      			IFEQR_($19)			      ;BNER_LAMP#19(double_score) 
      				SSND_($18)
      			      ...
      			      SLEEP_(70)
      			ENDIF_
      			SETRAM_($08,$14)			;RAM$08=$14
      			RSET1_($4D)				;Effect: Range #0D (Bounus 1-30)
...
bonus_loop		      	IFNER_($FB,$F0,$F5,$8D)	      ;BEQR_(RangeOFF#8D || TILT)
						...
						ENDIF_
	            			...
	            			IFNER_($F5,$80)		      ;BEQR_RangeOFF#80
	            			      RCLR1L1_($00)				;Effect: Range #00
	            			ELSE_			
	                                    RSET1_($00)				;Effect: Range #00
			...
			BITOFF_($7F)			;Set clear the bonus flag	
			CPUX_					;Resume CPU Execution
			rts	
...
;***********************************************************
;* Attract Mode Lamps 1: LORD Lamps
;***********************************************************
...
at2_loop			      BEQR_($FC,$FF,$E0,$01,$00,at2_1)	;BEQR_(LAMP#01(bip) & RAM$00)==#0 to at2_1
				...
				RROL1_($06,$05)			;Effect: Range #06 Range #05
				JMPR_(at2_2)			
at2_1			NEEND_($F7,$2A)			;BEQR_BIT#2A 
			RROR1_($06,$05)			;Effect: Range #06 Range #05
at2_2			RROL1_($0B)				;Effect: Range #0B
			SLEEP_(4)
			JMPR_(at2_loop)			
...
attract_3	jsr	macro_start
		PRI_($43)				;Priority=#43
      	;Run our new code if not set
      	BITON2_($58)			;Turn ON: Lamp#18(dt_right)
      	BITON2_($5C)			;Turn ON: Lamp#1C(dt_left)
      	BITON2_($5D)			;Turn ON: Lamp#1D(loop_dshield)
      	BEGIN_
...
;**********************************************************
;* Attract Mode Lamps 4
;*
;* This effect makes the super fast flashing effect on the
;* mini playfield and other lamps.
;**********************************************************				
attract_4	      jsr	macro_start
			...
			BEGIN_
				...
				BITINV2_($5E)			;Toggle: Lamp#1E(look_x)
				BITINV2_($5D)			;Toggle: Lamp#1D(loop_dshield)
			...
			LOOP_	
;**********************************************************
;* Attract Mode Lamps 5
;*
;* This effect sets and rotates a single lamp on through the
;* 1-5 number targets/rollovers.
;**********************************************************	
attract_5		jsr	macro_start
			PRI_($43)				;Priority=#43
			BITON2_($50)			;Toggle: Lamp#10(1_target)
			BEGIN_
				RROR1_($04)				;Rotate Right Lamp Group $04 (1-5 targets)
				SLEEP_(4)
			LOOP_
;*********************************************************
...
gameover_entry	jsr	macro_start
			...
			BEGIN_
				...
				BEGIN_
					...
					ADDRAM_(rega,-1)			;RAM$00-=1
				EQEND_($FC,$E0,$00)		;BNER_RAM$00==#0
				;new attract possible, but randomize a bit
				IFEQR_($F4,$05)
					JSRR_(atimeout_orig)
				ELSE_
					JSRR_(atimeout_new)
...
;***********************************************************
;* Used to start all the attract mode threads up.
;***********************************************************			
start_atthreads	NEWTHREAD(attract_1)
			NEWTHREAD(attract_2)
			NEWTHREAD(attract_3)
			NEWTHREAD(attract_4)
			NEWTHREAD_JMP(attract_5)
			
;***********************************************************
;* This is the orignal attract mode timeout, it flashes the 
;* GI and plays a random speech phrase. This is still used
;* but alternates with the new attract mode timeout code
;* which is below.
;***********************************************************		
atimeout_orig	SETRAM_(rega,$10)			;RAM$00=$10
			BEGIN_
				JSRR_(gi_off_inc)			
				SLEEP_(4)
				JSRR_(gi_on_dec)			
...
;***********************************************************
;* This is the new attract mode timeout, it turns off all 
;* lamps (controlled and GI) then does the up/down lamp
;* animation 3 times and then continues the standard lamp
;* animation for the attract mode.
;***********************************************************			
atimeout_new	REMTHREADS_($FF,$43)		;Remove Multiple Threads Based on Priority
			RCLR1_($08)				;Effect: Range #08
			RSET1_($48)				;Effect: Range #08
			JSR_(gi_off_inc)
			;Attract sound
			...
			BEGIN_
				...
				ADDRAM_($0B,-1)
			EQEND_($FC,$EB,$00)
			JSRD_(start_atthreads)	
			MRTS_

			

;attract_play	;start lamp playground			
;			jsr 	macro_start
;			;RSET1_($00)				;Turn on all lamps in group
;			RCLR0_($00)				;Turn off all lamps in group
;			;BITOFF2_($70)
;			;BITOFF2_($35)
;			;BITOFF2_($00)
;			BITON2_($5E)				;Turn ON: Lamp#1E(loop_x)
;			BEGIN_
;				BITINV2_($5E)
;				RSET1R1_($00)				;Fill Left without reset
;				;RSET1RC1_($00)				;Fill Left with Reset on Full
;				;RCLR1L1_($00)				;Clear Right without reset
;				;RROL1_($00)				;Rotate Left
;				;RROR1_($00)				;Rotate Right
;				;SLEEP_(30)
;				;RINV1_($00)				;Invert all lamps
;				SLEEP_(60)
;			LOOP_ 
...
;***************************************************
;* this is our special extended sound routine that
;* needs to be called for any of the sound commands
;* that are above $1F. Since the original level 7
;* flipper ROM's only support $1F commands, we need
;* to hack into the system calls to do this correctly.
;* When this routine is called A should contain the 
;* extended sound command from $00 to $1F, these will
;* translate into sound commands of $20-$3F
;***************************************************
ext_snd           ldab  #$01
                  psha	
			pshb	
			...
			ldaa	#$7F
			jsr	send_snd				;Send Sound Stop Command
			tba	                              ;get it back
			staa	soundindex				;Sound Command Table Index
			asla	
			aba						;Index * 3
			ldx	#soundtable_x			;Game ROM: Sound Table Pointer
			stx  	temp3
			jsr	xplusa				;X = X + A
			ldaa	$02,X	
			anda 	#$20
			ifne
				;wait a bit for the sound board to process the previous
				ldaa	#(~$1F)&$1F
				jsr	send_snd				;Send Bank Command
				;now wait again
				
			endif
			lds	temp3
			jmp   isnd_mult_x                  ;Jump to the middle of the regular sound
			                                    ;routine in the flipper ROM
			
;*****************************************************
;* Sound Lookup Table:
;*****************************************************
;  1st Byte: Sound Flags
;  2nd Byte: IRQ Counter
;  3rd Byte: Raw Sound Command      rawcmd;(sndcmd) (index)

#define	regsound(a,b,c)		.db	a,b,(~c)&$3F

soundtable		regsound($23,$06,$05)		;(00) Credit Sound
			regsound($A0,$04,$10)		;(01) Pop Bumper Thud
			regsound($28,$06,$05)		;(02) Credit Sound
			regsound($A0,$04,$07)		;(03) Thud
			regsound($22,$40,$0D)		;(04) UDT Bank Down
			regsound($28,$02,$12)		;(05) 
			regsound($24,$22,$02)		;(06) Electric Melt
			regsound($24,$50,$06)		;(07) Double Trouble Target Timeout
			regsound($C9,$10,$01)		;(08) Tilt
			regsound($23,$20,$0B)		;(09) UDT Bank Down
			.dw c_sound1\	.db $FF	;(0A) 
			regsound($27,$20,$0C)		;(0B) Credit
			regsound($28,$2C,$1C)		;(0C) High Score 
			regsound($27,$40,$14)		;(0D) Special (Long Slow Explosion)
			regsound($CC,$F0,$19)		;(0E) Add Player - Random Speech
			regsound($CE,$D0,$16)		;(0F) "Me Jungle Lord"
			regsound($CE,$FF,$18)		;(10) "You Win! Fight in Jungle Again"
			regsound($CE,$88,$11)		;(11) "You Jungle Lord"
			regsound($4E,$FF,$0F)		;(12) "Stampede, (trumpet)"
			regsound($CE,$FF,$0E)		;(13) "Fight Tiger Again"
			regsound($CE,$D8,$0A)		;(14) "Jungle Lord in Double Trouble" OR "You in Double Trouble"
			regsound($CE,$C8,$03)		;(15) High Score - "You  Win! You Jungle Lord"
			regsound($8C,$80,$16)		;(16) Match - "Me Jungle Lord"
			regsound($CE,$A8,$04)		;(17) Drop Target Timer
			regsound($05,$50,$1E)		;(18) Trumpet
			regsound($05,$50,$1E)		;(18) Trumpet
			regsound($24,$02,$09)		;(1A) Lower Drop Target Timer
			regsound($BE,$40,$15)		;(1B) Background Sound
			regsound($9F,$40,$13)		;(1C) Game Over (Kill All BG)
			regsound($A9,$60,$1D)		;(1D) Match Sound
			regsound($04,$04,$09)		;(1E) Bonus Count
			regsound($28,$40,$17)		;(1F) Explosion

c_sound1		.db $26,$82,$2D,$80,$3A,$3F	;(19)(12)(02)

;**********************************************************************
;* these are the extended sounds beyond $1F, they must be called via 
;* the routine 'ext_snd' with the index into this table in A
;**********************************************************************
soundtable_x	regsound($CE,$06,$2F)		;(00) monkey_oohooh
			regsound($CE,$04,$23)		;(01) tiger_double
			regsound($CE,$06,$21)		;(02) tiger_slow
			regsound($CE,$04,$29)		;(03) shriek1
			regsound($CE,$D0,$3A)		;(04) "Me Jungle Lord"
			regsound($CE,$D0,$35)		;(05) BK Laugh
			regsound($CE,$22,$3F)		;(06) Alt Bonus Countup 
			regsound($CE,$50,$00)		;(07) 
			regsound($CE,$10,$00)		;(08) 
			regsound($CE,$20,$00)		;(09) 
			regsound($CE,$20,$00)		;(0A) 
			regsound($CE,$20,$00)		;(0B) 
			regsound($CE,$20,$00)		;(0C) 
			regsound($CE,$20,$00)		;(0D) 
			regsound($23,$20,$00)		;(0A) 
			regsound($23,$20,$00)		;(0B) 
			regsound($23,$20,$00)		;(0C) 
			regsound($23,$20,$00)		;(0D) 
			regsound($23,$20,$3B)		;(0E) Drum Background
			regsound($23,$20,$1F)		;(0F) Trumpet
			;Multiball Animals
			regsound($23,$10,$20)		;(10) tiger_norm
			regsound($23,$10,$24)		;(11) tiger_bark
			regsound($23,$10,$28)		;(12) simple_chirp
			regsound($23,$10,$2A)		;(13) shriek2
			regsound($23,$10,$2B)		;(14) shriek3
			regsound($23,$10,$30)		;(15) monkey_oohooh2
			regsound($23,$10,$31)		;(16) monkey_oohooh3
			regsound($23,$10,$1F)		;(17) Trumpet
			;Attract Sounds
			regsound($23,$20,$22)		;(18) tiger_slowest
			regsound($23,$20,$2E)		;(19) monkeytune
			regsound($23,$20,$2C)		;(1A) shriek4
			regsound($23,$20,$36)		;(1B) trumpet_slower
			regsound($CE,$20,$32)		;(1C) monkey_oohooh4
			regsound($CE,$20,$33)		;(1D) monkey_oohooh5
			regsound($CE,$20,$37)		;(1E) fight_jungle_lord 
			regsound($CE,$20,$2D)		;(1F) shriek5
...
#if $>$E000
	.error "ROM Overflow at $E000, make sure your code is not too big."
#endif
 	.org $e000
...
sw_upper_sling	POINTS_(1,10)
			IFEQR_($1B)
				...
				EXEEND_
			ELSE_
				SSND_($01)				;10 Points/Digit Sound
			ENDIF_
			IFNER_($FB,$F6,$0C,$F5,$0C)	;BEQR_(RangeOFF#0C || RangeON#0C) 
			      RINV0_($0C,$0E)				;Effect: Range #0C
			ENDIF_
...
loop_forward	REMTHREADS_($F8,$50)		;Remove Multiple Threads Based on Priority
			...
			IFEQR_($F4,$1E)    	      ;BNER_LampOn/Flash#1E
				;yes
      			...
      			KILL_					;Remove This Thread
                  ENDIF_
                  ;was drain shield lit?
      		IFEQR_($F4,$1D)		      ;BNER_LampOn/Flash#1D
      			;yes
      			BITOFF_($1D)			;Turn OFF: Lamp#1D(loop_dshield)
      			BITOFF2_($1D)			;Turn OFF: Lamp#1D(loop_dshield)
      			JSRR_(loop_effect)			
			ELSE_		
				;no, nothing special
				SETRAM_(rega,$01)
      			JSRD_(ext_snd)	
      			POINTS_(15,1000)			;15000 Points
      			JMPR_(bit1b_off)	
      			;done
...
loop_backwards	SETRAM_(rega,$02)
      		JSRD_(ext_snd)
			JSRR_(loop_effect)			
			JMPR_(noloop)
						
loop_effect		RSET1R0_($0E)				;Effect: Range #0E
...
eject_common	      PRI_($15)				;Priority=#15
      			...
      			IFNER_($F8,$08)		      ;BEQR_SW#08
            			BITON2_($17,$1F,$25,$2E,$2F,$2B)	;Turn ON: Lamp#17(mini_pf)
...
minipf_done		IFEQR_($64)			      ;BNER_BIT#24
      			BITOFF2_($2B,$17,$1F,$25,$2E,$2F)	;Turn OFF: Lamp#2B(extra_kick)
...
mb_start		BITOFF_($41)			;Turn OFF: Bit#01
			RCLR0_($02,$0C)			;Effect: Range #02 Range #0C
...
;* this will start the timer timout process where the 
;* counter is paused for a bit and then starts going
;* again if the player doesn't shoot the ball.
;******************************************************						
mb_pause		SSND_($1C)				;Kill all BG Sound
			JSRR_(show_score)			
			SOL_(trough_on)   		;Turn ON Sol#2:trough
			EXE_
			      ldaa	game_ram_a
			...
			;switch to the background drums here...
			REMTHREADS_($F8,$E0)			;Kill the BG Sound thread
mb_nopause		SETRAM_(rega,$0E)
			JSRD_(ext_snd)
			;BEGIN_
			;	SETRAM_(regb,$05)			;RAM$01=$05
			;	SSND_($18)				;Sound #18
mb_loop		;	PRI_($B1)				;Priority=#B1
			...
			;SLEEP_(128)
			;JSR_(buzz_off_dec)
			SSND_($1B)				;Start normal background thread	
			IFNER_($FC,$D9,$01)	      ;BEQR_ADJ#9==#1
      			EXE_
      			      NEWTHREAD(bg_snd)			;Start the BG Sound
      			EXEEND_
...
double_trouble	PRI_($05)				;Priority=#05
			RCLR0_($09,$0B)			;Effect: Range #09 Range #0B
...
sw_dt_u5		IFNER_($42)		      ;BEQR_BIT#02
      			...
      			IFNER_($E0)		;BEQR_RAM$00
            			BITONP_(rega)			;Turn ON Lamp/Bit @RAM:00
            			JSRD_(add_bonus)			;1 bonus advance			
...
#if $>$E800
	.error "ROM Overflow at $E800, make sure your code is not too big."
#endif
...
lampm_g			
			...
			begin
				...
				bcs	b_098					;Branch if on first lamp in range
			neend
			tba	
			coma	
			anda	$00,X
...
lr_ret		jsr	unpack_byte				;(X = X + A>>3), B = (bitpos(A&07))
			tba	
			anda	$00,X
			rts						;Buffer now contains byte pos for start lamp
