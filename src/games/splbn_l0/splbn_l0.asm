...
;*                     $D000-$DFFF equal to $00
;***************************************************        
    
csum1       .chk $d000,$dfff,$80,C
...
;********************************************************           
comma_million   .db $40,$04 ;NOTUSED,$80,$08
comma_thousand  .db $10,$01 ;NOTUSED,$20,$02
...
set_comma_bit       ldaa    player_up               ;Current Player Up (0-1)
                    jsr     xplusa                  ;X = X + A
                    ldaa    $00,X
                    oraa    comma_flags
                    staa    comma_flags
...
;Test Passed Value in B agains the current player Display Mask (are we in Mask 0 or 1)
...
sys_irq_entry
                ...
                ifeq                ;Branch on Digits 2-8 or 10-16 (scores)
                    ...
                    ldab    irq_counter
                    stab    pia_disp_digit_data     ;Set the Digit 
                    beq     b_082
                    jmp     skip_lots             ;short IRQ on Blanking 
                endif
                ;here only when IRQ Counter != 0
                stab    swap_player_displays
                decb  
                ifne   
                    subb    #03
                    bne     clr_comma           ;Mask Away Comma
                endif
                rol     comma_data_temp
                rorb  
                rol     comma_data_temp
                rorb  
                orab    pia_comma_data
                bra     sav_comma           ;Save Comma Data
;Here on IRQ Counter == 0
b_082           inc     irqcount16
            ...
            ;* Clear Commas
            ;***********************************
clr_comma       ldab    pia_comma_data
                andb    #$3F
sav_comma       stab    pia_comma_data
            ...
            ;reset displays
                ...
                clr     pia_alphanum_segh_data      
                ldaa    #$FF
                staa    pia_disp_seg_data           ;clear the BCD output to display
                ldaa    irq_counter
                staa    pia_disp_digit_data         ;Set the Display Digit     
                ldaa    score_p1_b0,X               ;Load main Score data
                rol     credp1p2_bufferselect
                ifcs
                    ldaa    score_p1_b1,X               ;Load secondary Score data
                endif
                ;A now contains the value for the main score displays based on digit in irq_counter
                ldab    #03
                cmpb    irq_counter
                ifgt
                    rol     alpha_bufferselect
                ...
                ifcc
b_083               lsrb                        ;B BCD Values goes to high nybble
                    lsrb    
                    lsrb    
                    lsrb    
                    anda    #$F0
...
;*                     $E000-$EFFF qual to $00
;***************************************************
    
csum2           .chk $e000,$efff,$80,C
...
disp_mask       ldab    player_up               ;Current Player Up (0-1)
                ldx     #comma_million          ;Comma Tables
                jsr     xplusb              ;X = X + B)
                ldaa    $00,X                   ;comma_million: 40 04 NOTUSED: 80 08
                oraa    $04,X                   ;comma_thousand: 10 01 NOTUSED: 20 02
                rts 
                ...
                ifne                        ;No award if backup HSTD is 0,000,000
                    ...
                    begin
                        ...
                        begin
                            ...
                            bne hstd_adddig             ;Add LSB of sys_temp_w3 to sys_temp5 then continue
                            inx                     ;Next Digit
...
update_hstd     ldx #aud_currenthstd            ;Current HSTD
                ...
                staa    sys_temp2
                stx temp1
                bsr wordplusbyte            ;Add LSB of sys_temp_w3 to sys_temp5
                ldab    #$04
                dex 
...
hstd_adddig     bsr wordplusbyte            ;Add LSB of sys_temp_w3 to sys_temp5
